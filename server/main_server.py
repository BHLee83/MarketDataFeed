...
from server.market_data_processor import DataProcessor
...
class DataServer:
    def __init__(self, host=Config.HOST, port=Config.PORT, monitor=None):
        ...
        self.marketdata_meta = None  # 메타데이터 저장을 위한 변수 추가
    ...
    def start(self):
        try:
            ...
            # 메타데이터 로드 및 저장
            self.marketdata_meta = self.db_manager.load_marketdata_meta()
            server_logger.info("메타데이터가 로드되었습니다: %s", self.marketdata_meta)
    ...
    def send_data_to_db(self, current_time):
        ...
        if new_data:
            for data in new_data:
                prepared_data = self.data_processor.prepare_db_data(data)
    ...
    def check_idle_time(self):
        ...
        while self.running:  # running 플래그 확인
            try:
                ...
                # idle 상태 체크 (CPU 사용률 30% 이하, 메모리 여유 40% 이상)
                if cpu_usage <= 30 and memory_available >= 40:
                    # 시작 시간부터 10분 단위 경과 체크
                    elapsed_time = time.time() - start_time
                    if elapsed_time >= interval:
                        # DB 저장 로직 호출
                        self.send_data_to_db(time.time())
                        print(f"[{datetime.now()}] {interval}초 기간 실시간 데이터 보관 완료")
                        # 다음 10분 주기를 위해 시작 시간 갱신
                        start_time = time.time()
        ...
        while self.running:
            try:
                ...
                with MarketData.MarketData.from_bytes(data) as market_data:
                    ...
                    self.data_processor.process_data(market_data)
